...
vqi_pad_grninf_clean <- readRDS("P:/Pro00113988 - PAD Risk Models/DataRepo/VQI/Analysis Data Files/vqi_pad.rds")
...
source("P:/Pro00113988 - PAD Risk Models/Vasc Fem Inf/vasc_fem_inf/code/vasc_fem_inf_vqi_clean_infra_ltf.R")
...
source("P:/Pro00113988 - PAD Risk Models/Vasc Fem Inf/vasc_fem_inf/code/vasc_fem_inf_vqi_clean_supra_ltf.R")
...
label(vqi_pad_grninf_preop$GENDER) <- "Sex"
label(vqi_pad_grninf_preop$RACE) <- "Race"
label(vqi_pad_grninf_preop$SURGYEAR) <- "Operative Year"
label(vqi_pad_grninf_preop$indication) <- "Indication"
label(vqi_pad_grninf_preop$proc) <- "Procedure"
...
for (pred in predictors) {
  roc <- roc_curve(vqi_pad_grninf_preop, dep_1, pred, event_level = "second")
  auc <- roc_auc(vqi_pad_grninf_preop, dep_1, pred, event_level = "second")
...
# Loop through each predictor - need to indicate that the event level is second because TRUE comes after FALSE
for (pred in predictors) {
  roc <- roc_curve(vqi_pad_grninf_preop, dep_2, pred, event_level = "second")
  auc <- roc_auc(vqi_pad_grninf_preop, dep_2, pred, event_level = "second")
  auc_round <- round(auc$.estimate, 2)
...
combined_cal <- rbind(
  ...
  data.frame(model = "Souror", grninf = vqi_pad_grninf_preop$grninf, .pred_TRUE = vqi_pad_grninf_preop$s_nomo),
...
combined_cal <- rbind(
  ...
  data.frame(model = "Souror", grninf = vqi_pad_grninf_preop$s_grninf, .pred_TRUE = vqi_pad_grninf_preop$s_nomo),
...
saveRDS(vqi_pad_grninf_preop, file="P:/Pro00113988 - PAD Risk Models/DataRepo/VQI/Analysis Data Files/vqi_pad_grninf_preop.rds")
